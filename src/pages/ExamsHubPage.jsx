...
export default function ExamsHubPage() {
  ...
  // Labels used to collapse to icon-only below `sm:` to avoid crowding —
  // that left mobile with 3 unlabeled icons and no way to tell which mode
  // was which beyond a subtle selected-state color change. Labels are now
  // always visible; the row scrolls horizontally instead (same pattern as
  // HubTabBar's Notes/Flashcards/... tabs) if a narrow screen genuinely
  // can't fit everything, with a right-edge fade so a scrollable overflow
  // is visually obvious rather than looking like the full set of options.
    ...
    <>
      ...
      <div className="sticky top-0 z-30 bg-white/85 backdrop-blur-xl border-b border-slate-100">
        <div className="flex items-center gap-2 px-4 py-3">
          {/* Practice / Exam / Grade pill toggle */}
          <div className="relative flex-1 min-w-0">
            <div ref={scrollRef} className="flex gap-1 bg-slate-100 rounded-2xl p-1 overflow-x-auto scrollbar-hide">
            ...
            {canScrollRight && (
              <div className="pointer-events-none absolute right-0 top-0 bottom-0 w-10 bg-gradient-to-l from-slate-100 via-slate-100/80 to-transparent rounded-r-2xl" />
            )}
          </div>
      ...
      {/* ── Content ─────────────────────────────────────────── */}
      <div className={`mx-auto ${tab === 'practice' ? 'p-4 lg:p-6 max-w-xl' : 'p-4 lg:p-6 max-w-3xl'}`}>
