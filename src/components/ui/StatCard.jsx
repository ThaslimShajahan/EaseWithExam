 ...
 *
 * Layout follows a dense "status tile" pattern (icon chip + caps label on
 * one line, bold value below) rather than a big centered number — reads as
 * a clean instrument panel when several tiles sit in a grid together.
 *
 * Props:
...
export default function StatCard({
  ...
  return (
    <div
      className={`rounded-card p-4 ${surface} ${onClick ? 'cursor-pointer card-interactive' : ''} ${className}`}
      onClick={onClick}
    ...
    >
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2">
          <div className={`h-7 w-7 rounded-lg flex items-center justify-center shrink-0 ${dark ? iconBg.replace('-50', '-900/30') : iconBg}`}>
      ...
      {sub && <p className={`text-micro mt-1 ${subCls}`}>{sub}</p>}
    </div>
  );
}
