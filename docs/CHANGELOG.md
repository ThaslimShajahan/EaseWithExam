...
**Root gap (investigation)**: two checks already existed — `keyContradictsExplanation` (free cross-check against the explanation text) and `answerVerification.js`'s `verifyQuestions` (a second GPT-4o call that independently re-solves the question and compares to the key). Both only asked **"is the keyed option correct?"**, never **"is it the *only* correct option?"** A re-solver asked to verify "is 64 a perfect square" correctly says yes, agrees with the key, and the question passed clean through both checks — the gap wasn't pipeline-specific, it existed even where the strongest safety net was already active.

**Most exposed surface (investigation)**: `src/admin/AdminPaperGen.jsx`'s `handlePublish` computed the free check's `needs_review` and then never read it — not filtered, not shown to the admin — and never called `verifyQuestions` at all, unlike every other generation path (AI Practice, `src/lib/backgroundGeneration.js`). It publishes straight to Mock Test / Exam Center — real graded content.

**Part 1 — closed the root gap.** New `ambiguousOptionsReason()` in `src/lib/questionGen.js`: deterministic, free, checks perfect square/cube, prime, even, odd, and "multiple of N" (regex-extracted), with negation handling ("which is *not* a perfect square"). Wired into `toEngineFormat`'s existing free-check step, so it now runs automatically on every pipeline that already calls `toEngineFormat`. `answerVerification.js`'s `verifyOne` MCQ prompt changed from "give me one derived letter" to `{"correct_letters": [...]}` — every option the model derives as correct; more than one back is flagged the same way a wrong key is. 12 new unit tests across `questionGen.test.js` / `answerVerification.test.js`, including the exact reported case end to end through `toEngineFormat`.

**Part 2 — fixed AdminPaperGen.jsx.** `handlePublish` now runs `verifyQuestions` (fails open on an unexpected error, matching that check's own documented philosophy). Nothing flagged → publishes as before. Something flagged → new `AnswerReviewModal` opens instead of publishing: lists only the flagged questions with their reason, checkbox per question defaulting to **excluded**, footer shows how many will actually be published, admin can Cancel or Publish.

**Part 3 — retroactive sweep, honestly scoped.** `scripts/sanity-check-mcq-answers.mjs` extended with "multiple of N"; re-run against `pyq_questions`: 38 MCQ rows, zero matches — not a clean bill of health, just too small/narrow a sample. The real exposure was always live AI generation (ephemeral, not sweepable) — Parts 1-2 prevent new bad questions rather than chase every past one.

**Real authenticated E2E validation, not just unit tests.** Same real-identity discipline as this session's storage work (minted Firebase custom token, headless Playwright) — but this time driving the *actual* admin UI, not just calling RPCs through `page.evaluate`: signed into `adminAuth` as the real superadmin, cleared the genuine server-verified passcode screen digit by digit (no bypass), then (a) ran a real AI generation through the real Generate button, (b) seeded a deterministic ambiguous+clean question pair via a temporary dev-only hook and drove the real Publish flow — confirmed the actual `AnswerReviewModal` DOM rendered, the checkbox defaulted unchecked, and `published_tests` afterward held exactly the clean question, (c) re-ran with the checkbox explicitly checked — both questions saved, (d) a fully clean pair published directly with no modal ever appearing. **20/20 assertions passed** (one first-pass flake was a timing bug in the test script's own verification read, confirmed by immediate reproduction in isolation, fixed, reran clean). All `__TEST_%` `published_tests` rows deleted afterward — confirmed 0 residue. The temporary dev hook was fully reverted from `AdminPaperGen.jsx` after validation; `src/lib/devAuth.js` keeps one small permanent addition, `signInAdminWithMintedToken` (the `adminAuth`-bound sibling of its existing export), kept as reusable QA infrastructure by owner's request.

**Deployed** via the standard procedure (`docs/DEPLOY.md`). No DB migration — this fix is entirely client-side logic plus calls to already-existing RPCs.
...
**Delete manifest** — new `admin_delete_chapter_manifest(p_caller, p_id)` RPC (`assert_verified_admin`, no status restriction — draft/approved/superseded are all deletable at the DB layer; the extra guard for an approved manifest is client-side only, by design). `AdminChapterManifest.jsx`'s `load()` now also fetches `superseded` rows (previously excluded entirely — they were unreachable anywhere in this screen) into a small history list, each with its own Delete button. A shared confirm panel (modal, not `window.confirm` — this needed to be slower and stronger on purpose): checks `knowledge_base` for exact-match existing content (has a `book` column) and `study_notes` for subject-wide existing content (no `book` column exists on that table, so this count is necessarily approximate and labeled as such), requires typing the exact book/subject name, and adds an extra acknowledgement checkbox when the target is the live approved manifest. `ACTION.DELETE` added to `changelog.js` and to the live `changelog_action_check` CHECK constraint (built from the actual live constraint definition via `supabase db query --linked`, not guessed — that table's constraints aren't checked into any migration file in this repo, pre-dating it).

**Backup all data** — new `assert_verified_superadmin` (stricter than `assert_verified_admin`: requires `role = 'superadmin'` specifically, kept separate rather than adding a flag param since 14+ existing RPCs assume the admin-or-superadmin behavior) gating two new RPCs: `admin_start_data_export` (row-count summary across all 5 tables) and `admin_export_study_notes` (paginated, bypasses `study_notes_read`'s `is_published = true` RLS so drafts are included — the one table of the five that isn't already openly selectable). New `AdminBackup.jsx` under People & Audit → Backup: fetches `chapter_manifests`/`knowledge_base`/`pyq_questions`/`content_figures` directly (already open-read tables, no RPC needed) plus `study_notes` via the RPC, zips client-side with `fflate` (new dependency — investigated first: ~11k rows / ~159MB total, dominated by `knowledge_base`'s embedding vectors, well within what a browser can hold and zip; no server-side export needed), one JSON file per table + `manifest.json` with row counts and timestamp. `content_figures` rows carry `image_url` as a plain field like any other column — actual image bytes are never fetched, images stay servable from Supabase storage. `ACTION.EXPORT` added alongside `ACTION.DELETE` in the same constraint migration.

**Tested for real before deploying** — `scripts/test-delete-and-export-features.mjs`, same real-identity pattern as this session's other test scripts (minted Firebase custom token, headless Playwright, calls RPCs through the actual app code so `verified_uid()`'s JWT check is genuine, not forged). 18/18 passed: create/delete a throwaway draft, delete an *approved* throwaway manifest (proving the server has no status restriction), reject a spoofed `p_caller`, content-check correctly returns 0 on empty content and 294 on a real book with known `knowledge_base` rows, a plain `admin` account is rejected by both export RPCs (`42501`) while still passing the ordinary `assert_verified_admin` gate on an unrelated RPC (proving the rejection is superadmin-specific, not a broken test account). All test data cleaned up — confirmed zero `__TEST_%` rows left in `chapter_manifests` afterward.

**Deployed** via the standard procedure (`docs/DEPLOY.md`) — bundle `index-BOPv-iWo.js`, backed up the prior web root first, all pre/post verification steps (hash match, on-disk extract confirmation, permissions, live HTTP, per-route title+canonical content check) passed. `deploy_log` entry written for real this time via `admin_insert_deploy_log` (id `bc81e7b6-fa97-4996-8395-a0c02073181f`) — this session had live Firebase Admin SDK access via `scripts/firebaseAdmin.mjs`, unlike several earlier entries in this log that had to leave the RPC call prepared for the project owner to run. **Not yet committed** — same "don't commit unless asked" convention as recent entries.

**FOLLOW-UP, not fixed tonight**: found while testing that the `changelog` table has **no SELECT policy at all** for any client role (`anon` or `authenticated`) — RLS is enabled with two permissive INSERT policies but zero SELECT policies, so a browser session can never read its own audit trail back, superadmin included. Confirmed via `pg_policies` and `pg_class.relrowsecurity`. This is why `logChangeAsync()` (which does `.insert(...).select('id').single()`) fails outright with "new row violates row-level security policy" — Postgres raises that specifically because RETURNING can't satisfy a nonexistent SELECT policy on the just-inserted row. Plain `logChange()` (fire-and-forget, no `.select()`) is unaffected since it never requests a representation back — confirmed its inserts land correctly via a direct `supabase db query --linked` check. **Likely also affects `getEntityHistory()` and the Admin → Activity Log tab** (both do a plain `.select()` against `changelog`) — not confirmed live, but the same RLS gap would silently return zero rows there regardless of what's actually logged. Worth a look next session: needs a `changelog_select_admin` (or similar) policy scoped to active admins, or an RPC wrapper like the rest of the admin surface already uses.

---

## 2026-09-03 — Third occurrence tonight of the `chapter_manifests` `.maybeSingle()`-without-status-filter bug, in `bulk-load-unit-notes.mjs`; FOLLOW-UP (not done tonight): sweep for any other unfiltered query against this table

While using `scripts/bulk-load-unit-notes.mjs` to dry-run the Chemistry Part I fileOrdinal fix (see the entry above), `loadManifest()` (line ~201) and `processOneFile()`'s own separate manifest lookup (line ~242) both queried `chapter_manifests` by `(exam_type, subject, book)` with no status filter, then called `.maybeSingle()`. Once tonight's fix created the expected approved+superseded pair for this book, both queries threw `"JSON object requested, multiple (or no) rows returned"` — the identical bug shape already found and fixed twice tonight in `AdminChapterManifest.jsx`'s `load()` (see the Revise-flow entry) and `AdminContentIntake.jsx`'s `manifestRow` lookup (`:859-865`, whose own comment already names this exact failure mode). Fixed both spots in `bulk-load-unit-notes.mjs` the same way: `.in('status', ['draft', 'approved'])` in the query, then pick `approved` over `draft` in JS rather than trusting the DB to hand back exactly one row.

Checked at the same time, not found broken: `AdminChapterManifest.jsx` and `AdminContentIntake.jsx`'s own `chapter_manifests` queries are both already the fixed shape; this script's own one-off fix companions (`fix-chem-p1-fileordinal.mjs`, `approve-chem-p1-fileordinal.mjs`) filter by exact status already and don't assume a single row.

**Not done tonight, flagged for later**: this is the third time the same query shape has bitten in one night, which is a real signal something should sweep for it structurally rather than getting caught one call site at a time — e.g. a lightweight lint/grep check (or a Postgres view/RPC that always excludes `superseded`) rather than relying on every future caller remembering the rule by hand. No further scripts currently query `chapter_manifests` directly (grep for `from('chapter_manifests')` across `scripts/` and `src/` turns up only the five files checked here), so nothing else is known-broken right now — just worth a structural fix at some point instead of a fourth manual catch.

---

## 2026-09-03 — OPEN BUG (not fixed tonight): multi-file batch upload can't handle a file spanning multiple manifest chapters

Found on CBSE Class 12 Chemistry "CHEMISTRY PART I" (`combined` file_structure, `isUnit` Unit→Chapter shape): uploading `1_Solutions.pdf` — the full Unit 1 file, covering the Unit heading plus all 7 sub-chapters (1.1–1.7) — through the multi-file batch match-review screen forced picking ONE sub-chapter from a dropdown, instead of recognizing the file covers all 8 entries under Unit 1.

Root cause, traced end to end, not guessed: `AdminContentIntake.jsx` has two upload pipelines with different (and inconsistent) multi-entry handling.

- **Classic single-file/URL/Drive queue** (`handleProcess`, ~line 1385) is correct: it calls `candidatesForFile(approvedManifest, fileOrdinal, null)` (`src/lib/chapterManifest.js:227`), which returns every entry sharing the file's `fileOrdinal`, and passes the whole set to `extractNotesByManifest` — "N covered entries produce exactly N chapters," per its own comment. It only falls back to a single-entry admin picker when `candidatesForFile` finds *nothing*, not when it correctly finds several.
- **Multi-file batch "match & review" screen** (`handleMatchFiles` → `MatchReviewScreen` → `handleProcessConfirmed`, ~lines 1054-1245) is broken for this shape. It's built around a single scalar `chosenOrdinal` per file. `matchFileToManifest` (`src/lib/pageRangeMatch.js:93`) correctly detects the overlap and returns `confidence: 'multiple', suggested: [all 7 entries]` — but `handleMatchFiles` (line 1102) only pre-selects `chosenOrdinal` when `suggested.length === 1`, so a `'multiple'` result always drops to the manual dropdown. That dropdown (`MatchReviewScreen`, line 727-738) lists every leaf chapter individually with no multi-select. Even a manual pick doesn't help: `handleProcessConfirmed` (line 1194-1195) hard-codes `const covered = [entry];` — always exactly one entry, no matter how many the matcher found. There is no code path in this screen that can send more than one manifest entry to `extractNotesByManifest`.

Same shape as tonight's Revise investigation: correct, well-tested detection logic (`pageRangeMatch.test.js:100` even has a test titled *"multiple: detected range spans more than one manifest entry (a genuinely combined file)"*) sitting in front of a downstream pipeline that was never proven to act on what the detector found. That test only asserts on `matchFileToManifest`'s own return value — nothing exercises `MatchReviewScreen`/`handleProcessConfirmed` with a `'multiple'` row, and there is no test anywhere with `isUnit: true` rows going through the batch screen.

**Not fixed tonight** — deliberately, per the pattern of a same-night fix landing untested against real Unit-structured data and causing a second regression (see the two entries above). Real fix, when picked up: make `handleProcessConfirmed` (and the pre-selection in `handleMatchFiles`) route through `candidatesForFile` the same way `handleProcess` already does, so a `'multiple'` row can be confirmed as the whole matched set rather than forced to one ordinal — then a permanent regression fixture combining `isUnit: true` + a multi-entry `combined` file through the actual batch screen, not just through `pageRangeMatch.js` in isolation, before calling it fixed.

**Workaround in use for now**: the classic single-file/URL/Drive queue (`handleProcess`) already handles this correctly, so Chemistry Part I's remaining chapters are being uploaded one file at a time through that screen instead of the batch match-review screen — confirmed first that every sub-chapter under a given Unit shares the same `fileOrdinal` in the approved manifest (the signal `candidatesForFile` actually matches on), since that path's correctness depends on it.

**That confirmation check found a second, separate real bug — bad manifest data, not code**: every one of the 34 sub-chapters had a DISTINCT `fileOrdinal` (1.1=1, 1.2=2, 1.3=3...), not the shared-per-unit value the real source files require (`1_Solutions.pdf` = Unit 1 heading + all 7 sub-chapters in one PDF). `candidatesForFile` matches by exact `fileOrdinal` equality, so as-approved it could never have grouped a unit's children under one file even via the working classic upload path — the single-file workaround would have hit the same "no match" wall.

Fixed as a manifest **data correction**, same discipline as the entry-4 `pageEnd` fix below: went through the real `admin_upsert_chapter_manifest` → `admin_approve_chapter_manifest` RPC path headless (Playwright + a minted Firebase custom token, `scripts/fix-chem-p1-fileordinal.mjs` then `scripts/approve-chem-p1-fileordinal.mjs`), never a direct DB mutation. Grouped the 34 children by their existing `unit` field against each Unit container (sorted by `ordinal`, so Unit 1's container → target `fileOrdinal: 1` … Unit 5 → `5`), sanity-checked the group sizes (7/8/5/7/7 children) against what was reported live and aborted-without-writing on any mismatch (none occurred — every `unit` field matched a container `title` exactly), ran `validateManifest` clean (0 errors) on the corrected 39-entry array, created a new draft (verified: does not touch the live approved row or the upload gate), showed the full per-row diff (29 of 34 rows changed — the other 5 already happened to have `fileOrdinal` equal to their own unit number, e.g. `1.1`, `2.2`, `3.3`, confirming the transform is exactly "child's position within its unit" → "unit number", not some other grouping) for explicit review before approving. Approved only on go-ahead; verified live afterward with a plain read (not trusting the RPC's own success response): new row `9200ccb5-158b-47bc-9e4a-6f98d0a55937` is `status=approved`, old row `5f02f77b-60fa-491b-996b-4751c955bed5` is `status=superseded` (kept, not deleted, approved 2026-09-02 by a different admin UID than tonight's), and all 34 live child rows now carry the correct per-unit `fileOrdinal`.

Noted but out of scope, not touched: several titles carry doubled/tripled internal whitespace (e.g. "Unit 1" + 3 spaces + "Solutions", "d -and" + 3 spaces + "f -Block") — looks like a pre-existing OCR/extraction artifact, present identically in both the container `title` and the children's `unit` field (which is why matching still worked), unrelated to this fix.

**No code change, no deploy** — this was a data-only fix through existing, already-approved RPCs on the shared Supabase backend; the local dev server used to drive the headless browser session never touched production EaseWithExam.

---

## 2026-08-30 — Deployed: manifest "Revise" flow, plus a live data fix on Kerala English Class 8 (not yet committed)

Follow-up upload error on the same Kerala State Class 8 English ("Wings of Hope") manifest: `"Chapter 6 The Day My World Changed.pdf"` was refused as `"claims printed pages 55-72 ... outside this 17-page file"`. Confirmed `file_structure` on the live row is `combined` (the error can only fire on that path — `extractNotesByManifest` skips the check entirely for `per_chapter`), so this was a real page-range typo, not a structure mismatch. Laid out all 12 entries by unit: entry 4 ("The Day My World Changed") had `pageEnd: 72` (18 printed pages) against a 17-page file. Two independent, convergent signals pointed at `pageEnd` specifically: every other chapter boundary in the manifest hands off contiguously (`pageEnd` of one = `pageStart` − 1 of the next), and in the book's other two units the last chapter inside a unit ends exactly 1 page before the unit container's own `pageEnd` — Unit II's last chapter was the only one ending flush with its unit's boundary. Correcting to `pageEnd: 71` satisfies both patterns and matches the file's real page count exactly.

Separately investigated a request to delete an *approved* manifest from the admin UI (currently impossible — "Approved manifests are read-only here", no path forward at all). Found: `study_notes`/`knowledge_base` carry no FK to `chapter_manifests` — they're matched by `(exam_type, subject, chapter/chapter_key/book)` text fields, so deleting a manifest would orphan nothing already loaded; the real damage would be to *future* admin work, since `requireApprovedManifest()` blocks all new uploads for that book until a manifest exists again, with no audit trail (`approved_by`/`approved_at`) surviving the deletion. Found the actual mechanism already exists at the DB layer and had zero callers: `admin_upsert_chapter_manifest` refuses to edit an approved row in place by design, and `admin_approve_chapter_manifest` already auto-supersedes the prior approved row for the same `(exam_type, subject, book)` on approval — "revise" (new draft, edited, re-approved) was always the intended path, just never wired to a UI button. Decided against building delete at all; built Revise instead.

**`AdminChapterManifest.jsx`**: added a "Revise" button on an approved manifest's status banner — sets the loaded row to `null` (entries/file structure/prefix/source file left as-is) so the next Save inserts a fresh draft seeded from the approved one, rather than trying to update it in place. Also fixed `load()`, which queried by `(exam_type, subject, book)` with no status filter and `.maybeSingle()` — the exact bug `AdminContentIntake.jsx`'s `manifestRow` lookup already hit and fixed once (its own comment explains it): a book mid-revision can briefly have BOTH a live approved row and a new draft, and once approved the old row becomes `superseded` rather than deleted, so an approved+superseded pair sits there forever after. `.maybeSingle()` errors on >1 row either way. Now excludes `superseded` and picks `approved` over `draft` in JS, matching `AdminContentIntake.jsx`'s pattern exactly.

Proved the latent bug was real by using the actual fix as the test case: ran the entry-4 correction through the real RPC path (`admin_upsert_chapter_manifest` with `p_id: null` → `admin_approve_chapter_manifest`, headless via Playwright + a minted Firebase admin token, mirroring `scripts/bulk-load-unit-notes.mjs`'s own auth pattern) against the live DB. Confirmed: the *old* `load()` query now throws `"multiple (or no) rows returned"` on that exact key once the approved+superseded pair exists; the *new* query correctly resolves to the approved row with `pageEnd: 71`. Full suite (603/603) and production build both clean.

**Deployed to production**, standard 8-step procedure. Backup taken first (`webroot-2026-08-30-080348.tar.gz`). scp exit 0, md5 matched both ends (`0afef27d848141086979c82900707885`). Extract hit the documented benign `tar` exit 2 (Gotcha 1); confirmed genuine via the disk check. Permissions fixed (0 unreadable files). All 5 prerendered routes content-checked (Gotcha 4) — each still serves its own title and canonical. Bundle hash `index-D7gwTc_i.js` confirmed live via direct HTTP, and `AdminChapterManifest-BBML8Idf.js` confirmed individually reachable (200). No DB migration for the code change; the entry-4 correction is a data fix made through the existing approved RPCs, not a schema change — nothing to push via `supabase db push`.

**Not committed** — staged for the project owner, same pattern as the previous two entries; deployed ahead of the commit, on explicit instruction.

---

## 2026-08-27 — Deployed: `isUnit` + `numbered:false` false-positive on the real Kerala English Class 8 manifest (not yet committed)

Same-day follow-up to the `isUnit` fix (commits `52cdcd6`, `4bb02fd`). That fix shipped on 7 synthetic tests but was never run against this book's actual shape, and missed a second bug: every Unit heading on the real manifest comes back `numbered: false` (a Unit heading has no printed CHAPTER number, which the extraction prompt's own wording for `numbered` reads just as naturally as "not numbered in the sequence" for), and `validateManifest` branched on `numbered` BEFORE `isUnit` — so `{isUnit: true, numbered: false}` fell into the INTERLEAVED branch, which demands a single leaf chapter's range fully contain it. Impossible for a heading spanning several chapters. Reproduced exactly against the pre-fix code (`52cdcd6`) with a Kerala-shaped fixture: `interleaved "Unit I/II/III" ... sits inside no numbered chapter`, verbatim what was reported live.

Fixed by making `isUnit` override `numbered` everywhere it's read in `chapterManifest.js` (classification, the interleaved-host check, `pagesOptionalHere`, and `candidatesForFile`'s `inside` filter — same latent bug there, unreachable until now but real for a `combined` book where one file can legitimately be an entire unit's pages). Fixed at the source too: `manifestExtraction.js`'s `normaliseEntries` now forces `numbered: true` whenever `isUnit: true` regardless of what the model returns, the prompt text was clarified, and the admin UI's "Unit row?" checkbox now forces the same when hand-toggled — this exact combination can no longer be produced by any of the three paths that write an entry.

Also investigated (asked, not assumed) whether `page_start`/`page_end` are load-bearing before touching anything: yes. `extractNotesByManifest` (`contentExtraction.js`) uses them to literally slice which pages of a `combined`-structure PDF get sent to the AI per chapter; `study_notes.page_start/page_end` is copied straight from the manifest's own values (not independently model-detected) for every manifest-driven upload; and both fields are also a student-facing "pg X–Y" badge and a secondary sort key in `NotesBrowser.jsx`/`AdminStudyNotes.jsx`. Kerala English Class 8 (one PDF per Unit, several chapters each) is almost certainly `combined`, where these are the only signal scoping extraction per chapter. Recommendation: keep the requirement — the bug was purely in overlap/containment validation, never in whether page numbers should be required.

Added a permanent regression fixture in `chapterManifest.test.js` — 3 units, `numbered:false` on every heading, ordinal 11 "The Astronomer" carrying the reported half-filled page range — reconstructed to match what was reported (not a byte-exact export of the real manifest; true up titles/pages if the real entries JSON is ever available). "The Astronomer"'s missing `pageEnd` itself is not a code bug — `combined` structure correctly requires both page numbers, same half-filled-range check that already existed; that one just needs the real page number typed in. Full suite: 603/603 passing. Production build clean.

**Deployed to production**, standard 8-step procedure. Backup taken first (`webroot-2026-08-27-122901.tar.gz`). scp exit 0, md5 matched both ends (`ccd7e04e5aa56e103b785128486f8567`). Extract hit the documented benign `tar` exit 2 (Gotcha 1); confirmed genuine via the disk check. Permissions fixed (0 unreadable files). All 5 prerendered routes content-checked (Gotcha 4) — each still serves its own title and canonical. Bundle hash `index-jjiGpvmv.js` confirmed live via direct HTTP, and the three feature-specific chunks (`AdminChapterManifest-CWE06owe.js`, `AdminContentIntake-CdX0l1EV.js`, `manifestExtraction-Choj6eH_.js`) confirmed individually reachable (200). No DB migration in this change — pure client-side validation/prompt logic, nothing to push to Supabase.

**Not committed** — staged for the project owner, same as the previous entry; deployed ahead of the commit again, on explicit instruction.

---

## 2026-08-27 — Deployed: false-positive "page ranges overlap" for Unit -> Chapter manifests (commit `52cdcd6`)

Chapter manifest validation (`validateManifest`, src/lib/chapterManifest.js) treated every entry as a flat sibling, so any book whose contents page prints a Unit/Theme heading WITH ITS OWN page range spanning several real chapters underneath it (Kerala State English Class 8: "Unit II Wings of Hope" pp41-72 containing "Hope is the Thing with Feathers" pp43-46) got the expected containment flagged as a false overlap, blocking Save/Approve. Checked live data first: 0 of the 20 existing `chapter_manifests` rows are actually affected — none currently model a Unit as its own row (confirmed by scanning every row's `entries` for any containment pair; none exist). The bug is real but had not yet been hit in saved data, only in a manifest someone was trying to draft.

No existing field distinguished a "this row IS the printed heading" container from a real leaf chapter — `unit` was (and still is) only a grouping-heading *label* on leaf entries. Added a new `isUnit: true` entry flag. A container's own children are whichever leaf entries already carry `unit === container.title` — no new linkage field needed. `validateManifest` now: skips overlap-flagging between a container and its own children; still flags overlap between two leaf chapters, between a container and a leaf that is NOT its child, and between two containers; new checks that a container's range actually contains each of its children (catches a typo'd page number) and that every container has at least one child pointing back to it (catches a title/unit string mismatch) — the latter works even in a `per_chapter` manifest with no page numbers on any entry, since it doesn't depend on ranges. page_start/page_end are untouched — still tracked, still used elsewhere (PDF extraction scoping).

A container is not a real chapter, so every other place that treated "any `numbered !== false` entry" as one had to learn to also exclude `isUnit`: `candidatesForFile` and `inferFileStructure` (chapterManifest.js), `matchFileToManifest` (pageRangeMatch.js — otherwise a container's range would spuriously double-match a file that really belongs to one of its children, turning a clean single match into a false "spans multiple chapters"), and the two manual chapter-picker dropdowns in AdminContentIntake.jsx. `manifestExtraction.js`'s draft-from-PDF prompt now tells the model when to emit `isUnit: true` (only when the book prints the heading with its own page number) so future drafts of Unit-structured books don't reproduce the bug; `normaliseEntries` never defaults a fileOrdinal onto a container (it has no file of its own). AdminChapterManifest.jsx gained an admin-facing "Unit row?" checkbox column, `isUnit` on `blankEntry`, and excluded containers from the client-side missing-File#-blocks-approval mirror check.

Also fixed the DB-side approval gate: `admin_approve_chapter_manifest` (20260815030000) independently refuses to approve any manifest with a null `fileOrdinal` on a `numbered` entry — a container legitimately has none, so without a matching fix the JS validator would pass but Approve would still be refused at the RPC. New migration `20260827000000_chapter_manifest_unit_container.sql` adds the same `isUnit` exclusion to that gate's WHERE clause and updates the `entries` jsonb column comment. **Pushed to the live linked project** (`efrhurxnlkthlkqeyiva`) via `supabase db push`, confirmed by reading back the live function body. Verified live with a direct SQL query mirroring the gate's exact WHERE clause against a Wings-of-Hope-shaped fixture: old logic (no `isUnit` exclusion) returns `#1 ("Unit II Wings of Hope")` as blocking; new logic returns `null` — nothing blocks it.

7 new tests in `chapterManifest.test.js` (34/34 passing in that file): accepts full containment, still rejects a genuine leaf-vs-leaf overlap, still rejects a leaf overlapping a unit it doesn't belong to (a case leaf-vs-leaf checking alone would miss, since the stray leaf sits in the gap between the unit's own children), still rejects two overlapping unit containers, flags a container that doesn't actually contain one of its declared children, flags an orphan container with no matching child, confirms the orphan check doesn't need page numbers under `per_chapter`, and confirms `candidatesForFile`/`inferFileStructure` exclude containers. Full suite: 599/599 passing. Production build clean.

**Deployed to production** the same session, standard 8-step procedure. Backup taken first (`webroot-2026-08-27-112522.tar.gz`). scp exit 0, md5 matched both ends (`d2d1280e5fca08f6db5a763ba3666cfb`). Extract hit the documented benign `tar` exit 2 (Gotcha 1); confirmed genuine via the disk check (`index.html` names the new hash, the file exists). Permissions fixed (0 unreadable files). All 5 prerendered routes content-checked (Gotcha 4) — each still serves its own title and canonical, not the homepage's. Bundle hash `index--c3xZjh9.js` confirmed live via direct HTTP, and the three feature-specific chunks that actually carry this fix (`AdminChapterManifest-B7L5E826.js`, `AdminContentIntake-CMFeXd4x.js`, `manifestExtraction-BqoWWFB4.js`) confirmed individually reachable (200) on the live server. Not a migration-paired deploy — the DB migration (previous entry) is backward-compatible (same RPC signature), so no special ordering was needed; it was already live before this deploy.

**Sequencing note**: this deploy happened BEFORE commit `52cdcd6` existed — the reverse of this repo's usual commit-then-deploy order, done on explicit instruction. The deploy is not somehow ahead of or out of sync with what's in git now; the commit is simply catching the repo up to what production already had running.

**Still not committed** — code changes, the new migration, and this deploy are all staged for the project owner to review and commit, per this project's "don't commit unless asked" convention. Whoever commits should note the deploy already happened *before* the commit exists, which is unusual for this log's normal order (commit, then deploy) — worth a comment in the commit message so a future reader isn't confused about sequencing.

Not committed — code changes and the new migration are on disk (migration is live in the DB), staged for the project owner to review and commit per this project's "don't commit unless asked" convention.
